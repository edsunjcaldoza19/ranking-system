                        ...
                        <div class="card-header">
                            ...
                            <?php
                                                ...
                                                while($fetchPopClass = $sqlPopClass->fetch()){
                                                    ...
                                                    $sqlClass = $conn->prepare("SELECT CAST(AVG(gbranch_grade) AS DECIMAL (10,2)) FROM tbl_grade_branch
                                                    LEFT JOIN tbl_subject_branch ON
                                                    tbl_subject_branch.id = tbl_grade_branch.gbranch_subject_id
                                                    WHERE gbranch_stud_id = $fetchStudentID
                                                    AND sbranch_class_id = $getClassID
                                                    AND sbranch_subject_id = $getSubjectID");
                                                    $sqlClass->execute();
                                                    while($fetchClass = $sqlClass->fetch()){
                                                        $studentArray[] = array(
                                                            'studentName' => $fetchPopClass['stud_name'],
                                                            'studentGrade' => $fetchClass['CAST(AVG(gbranch_grade) AS DECIMAL (10,2))']
                                                        );
                                                    }
                                                }
